...
fracking_clean <- fracking |> 
  ...
  # clean column names ----
  janitor::clean_names() |> #convert from uppercase to snakecase "-". Better readability of headers
  
  # clean up dates ----
  mutate(job_start_date = str_remove(job_start_date, " AM")) |> # remove string 'AM' from job_start_date_column
  mutate(datetime_start = mdy_hms(job_start_date)) |> # convert column from string to datetime type (saves as new column (datetime_start). Takes info from job_start_date
  ...
  filter(!str_detect(string = sn, pattern = "\\?")) |> # `?` is a special chr; escape with `\\` prefix, find strings that arent question mark pattern and remove them
