...
**Use `geom_col()` when your data is already summarized**
or you have a variable in your data set that includes y-axis values,
which will map to the height of the bars. E.g. we already have a numeric
variable in our data set called, `total_earnings` – those numeric values are mapped to the height of each bar in our plot.
...
**Use `geom_bar()` if you want to ggplot to count up numbers of rows**
and map those counts to the height of bars in your plot. E.g. we want
to know how many occupations are included for each major category in our
`jobs_gender_clean` data set (NOTE: we don’t have a `count` column in our data frame):
