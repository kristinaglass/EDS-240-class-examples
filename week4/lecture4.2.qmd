...
stream_chem_data %>% 
  ...
  ggplot(aes(x = DOC, y = pH)) +
  geom_point(alpha = 0.5) +
  geom_smooth(method = "lm", se = FALSE) #locally estimated scatter plot smoothing, similar to movig average fllowing trend of the points if you have less than 1k datapoints
```
```{r}
stream_chem_data %>% 
  filter(waterYr == 2021) %>% 
...
ggplot(stream_chem_data, aes(x = SO4, y = pH)) +
  geom_hex() +
  scale_fill_viridis_c() +
  guides(fill = guide_colorbar(title = "count", barwidth = 1,
                               barheight = 15))
```
